...
class MAXQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        print(
            get_parameters_num(
                list(self.mixer.parameters()) + list(self.central_mixer.parameters())
            )
        )
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        chosen_action_qvals_agents = torch.gather(
        ...
        if self.args.double_q:
            ...
            target_max_agent_qvals = torch.gather(
        ...
        central_mac_out = torch.stack(central_mac_out, dim=1)  # Concat over time
        central_chosen_action_qvals_agents = torch.gather(
            central_mac_out[:, :-1],
            dim=3,
            index=actions.unsqueeze(4).repeat(
                1, 1, 1, 1, self.args.central_action_embed
            ),
        ).squeeze(
            3
        )  # Remove the last dim
        ...
        central_target_mac_out = torch.stack(
            central_target_mac_out[:], dim=1
        )  # Concat across time
        # Mask out unavailable actions
        central_target_mac_out[avail_actions[:, :] == 0] = -9999999  # From OG deepmarl
        # Use the Qmix max actions
        central_target_max_agent_qvals = torch.gather(
            central_target_mac_out[:, :],
            3,
            cur_max_actions[:, :]
            .unsqueeze(4)
            .repeat(1, 1, 1, 1, self.args.central_action_embed),
        ).squeeze(3)
        # ---
        ...
        ws = torch.ones_like(td_error) * self.args.w
        if self.args.hysteretic_qmix:  # OW-QMIX
            ws = torch.where(
                td_error < 0, torch.ones_like(td_error) * 1, ws
            )  # Target is greater than current max
            ...
            ws = torch.where(
                is_max_action | qtot_larger, torch.ones_like(td_error) * 1, ws
            )  # Target is greater than current max
        ...
        agent_norm = agent_norm ** (1.0 / 2)

        mixer_norm = 0
        for p in self.mixer_params:
            param_norm = p.grad.data.norm(2)
            mixer_norm += param_norm.item() ** 2
        mixer_norm = mixer_norm ** (1.0 / 2)
    ...
    def load_models(self, path):
        ...
        self.optimiser.load_state_dict(
            torch.load(
                "{}/opt.th".format(path), map_location=lambda storage, loc: storage
            )
        )
