...
class LICALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length - 1):
            ...
            agent_probs = torch.nn.functional.softmax(agent_outs, dim=-1)
            mac_out.append(agent_probs)
            mac_out_entropy.append(agent_entropy)
        mac_out = torch.stack(mac_out, dim=1)  # Concat over time
        mac_out_entropy = torch.stack(mac_out_entropy, dim=1)
    ...
    def train_critic_td(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if (
            self.critic_training_steps - self.last_target_update_episode
