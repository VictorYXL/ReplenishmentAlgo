...
from utils.rl_utils import build_target_q, build_td_lambda_targets
...
class OffPGLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, log):
        ...
        baseline = torch.sum(mac_out * q_vals, dim=-1).view(-1).detach()
        ...
        coma_loss = (
            -((coe * advantages.detach() * log_pi_taken) * mask).sum() / mask.sum()
        )
    ...
    def train_critic(self, on_batch, best_batch=None, log=None):
        ...
        targets_taken = self.target_mixer(
            torch.gather(target_q_vals, dim=3, index=actions).squeeze(3), states
        )
        ...
        if best_batch is not None:
            (
                ...
                best_actions,
                best_mac_out,
            ) = self.train_critic_best(best_batch)
            log["best_reward"] = torch.mean(
                best_batch["reward"][:, :-1].squeeze(2).sum(-1), dim=0
            )
            target_q = torch.cat((target_q, best_target_q), dim=0)
            inputs = torch.cat((inputs, best_inputs), dim=0)
            mask = torch.cat((mask, best_mask), dim=0)
            actions = torch.cat((actions, best_actions), dim=0)
            states = torch.cat((states, best_batch["state"]), dim=0)
        ...
        for t in range(max_t - 1):
            ...
            q_ori = q_vals
            q_vals = torch.gather(q_vals, 3, index=actions[:, t : t + 1]).squeeze(3)
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                self.c_params, self.args.grad_norm_clip
            )
            ...
            log["q_max_mean"].append(
                (torch.mean(q_ori.max(dim=3)[0], dim=2, keepdim=True) * mask_t)
                .sum()
                .item()
                / mask_elems
            )
            log["q_min_mean"].append(
                (torch.mean(q_ori.min(dim=3)[0], dim=2, keepdim=True) * mask_t)
                .sum()
                .item()
                / mask_elems
            )
            log["q_max_var"].append(
                (torch.var(q_ori.max(dim=3)[0], dim=2, keepdim=True) * mask_t)
                .sum()
                .item()
                / mask_elems
            )
            log["q_min_var"].append(
                (torch.var(q_ori.min(dim=3)[0], dim=2, keepdim=True) * mask_t)
                .sum()
                .item()
                / mask_elems
            )
    ...
    def train_critic_best(self, batch):
        ...
        with torch.no_grad():
            ...
            for i in range(max_t):
                ...
                mac_out.append(agent_outs)
            mac_out = torch.stack(mac_out, dim=1).detach()
            # Mask out unavailable actions, renormalise (as in action selection)
            mac_out[avail_actions == 0] = 0
            mac_out = mac_out / mac_out.sum(dim=-1, keepdim=True)
            mac_out[avail_actions == 0] = 0
            critic_mac = (
                torch.gather(mac_out, 3, actions).squeeze(3).prod(dim=2, keepdim=True)
            )
            ...
            # td-error
            targets_taken[:, -1] = targets_taken[:, -1] * (
                1 - torch.sum(terminated, dim=1)
            )
            exp_q[:, -1] = exp_q[:, -1] * (1 - torch.sum(terminated, dim=1))
            targets_taken[:, :-1] = targets_taken[:, :-1] * mask
            exp_q[:, :-1] = exp_q[:, :-1] * mask
            td_q = (
                rewards + self.args.gamma * exp_q[:, 1:] - targets_taken[:, :-1]
            ) * mask

            # compute target
            target_q = build_target_q(
                td_q,
                targets_taken[:, :-1],
                critic_mac,
                mask,
    ...
    def build_exp_q(self, target_q_vals, mac_out, states):
        target_exp_q_vals = torch.sum(target_q_vals * mac_out, dim=3)
    ...
    def save_models(self, path):
        ...
        torch.save(self.mixer_optimiser.state_dict(), "{}/mixer_opt.th".format(path))
    ...
    def load_models(self, path):
        ...
        self.mixer.load_state_dict(
            torch.load(
                "{}/mixer.th".format(path), map_location=lambda storage, loc: storage
            )
        )
