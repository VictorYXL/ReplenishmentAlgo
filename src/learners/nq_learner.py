...
class NQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        if self.use_per:
            per_weight = (
                torch.from_numpy(per_weight).unsqueeze(-1).to(device=self.device)
            )
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("mask_elems", mask_elems, t_env)
        ...
        if self.use_per:
            if self.return_priority:
                ...
                self.priority_max = max(
                    torch.max(info["td_errors_abs"]).item(), self.priority_max
                )
                self.priority_min = min(
                    torch.min(info["td_errors_abs"]).item(), self.priority_min
                )
                ...
                info["td_errors_abs"] = (
                    ((td_error.abs() * mask).sum(1) / torch.sqrt(mask.sum(1)))
                    .detach()
                    .to("cpu")
                )
        return info
