...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.mixer == "qtran_base":
            ...
            td_targets = (
                rewards.reshape(-1, 1)
                + self.args.gamma * (1 - terminated.reshape(-1, 1)) * target_joint_qs
            )
            ...
            if (
                not self.args.double_q
            ):  # Already computed if we're doing double Q-Learning
                max_actions_current_ = torch.zeros(
                    size=(
                        batch.batch_size,
                        batch.max_seq_length,
            ...
            opt_error = (
                max_actions_qvals[:, :-1].sum(dim=2).reshape(-1, 1)
                - max_joint_qs.detach()
                + vs
            )
            ...
            nopt_values = (
                chosen_action_qvals.sum(dim=2).reshape(-1, 1) - joint_qs.detach() + vs
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.args.mixer == "qtran_base":
                ...
                self.logger.log_stat(
                    "td_targets", ((masked_td_error).sum().item() / mask_elems), t_env
                )
                self.logger.log_stat(
                    "td_chosen_qs", (joint_qs.sum().item() / mask_elems), t_env
                )
