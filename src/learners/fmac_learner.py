...
class FMACLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, off=False):
        ...
        with torch.no_grad():
            ...
            target_vals = build_td_lambda_targets(
                rewards,
                terminated,
                mask,
                target_vals,
        ...
        critic_loss = (
            0.5 * ((q_taken - target_vals.detach()) * mask).pow(2).sum() / mask.sum()
        )
        ...
        if not off:
            ...
            pi = torch.stack(pi, dim=1)  # Concat over time b, t, a, probs
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat(
                "target_vals",
                (target_vals * mask).sum().item() / mask.sum().item(),
                t_env,
            )
            ...
            if not off:
                ...
                self.logger.log_stat(
                    "pi_max",
                    (pi.max(dim=-1)[0] * agent_mask).sum().item()
                    / agent_mask.sum().item(),
                    t_env,
                )
    ...
    def save_models(self, path):
        ...
        torch.save(self.agent_optimiser.state_dict(), "{}/opt.th".format(path))
