    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        with torch.no_grad():
            ...
            advantages, targets = build_gae_targets(
                rewards, mask_agent, values, self.args.gamma, self.args.gae_lambda,
            )
        ...
        for _ in range(self.args.mini_epochs):
            # Critic
            # self.local_critic.input_seq_str = f"o_la^i_{self.n_agents}_{self.n_actions}"
            # self.local_critic.n_agents = self.n_agents
            if "rnn" in self.args.critic_type:
                values = []
                # local_values = []
                self.critic.init_hidden(batch.batch_size)
                # self.local_critic.init_hidden(batch.batch_size)
                for t in range(batch.max_seq_length - 1):
                    agent_outs = self.critic.forward(batch, t=t)
                    values.append(agent_outs)
                    # agent_outs_v2 = self.local_critic.forward(batch, t=t)
                    # local_values.append(agent_outs_v2)
                values = torch.stack(values, dim=1)
                # local_values = torch.stack(local_values, dim=1)
            else:
                values = self.critic(batch)[:, :-1]
                # local_values = self.local_critic(batch)[:, :-1]
            ...
            # self.local_critic.input_seq_str = f"o_la^i_1_{self.n_actions}"
            # self.local_critic.n_agents = 1
            ...
            # regularization
            # reg_loss = (((values.mean(2)-local_values.sum(2))*mask)**2).sum()/mask.sum()

            loss = (
                actor_loss
                + self.args.critic_coef * critic_loss
                # + self.args.reg_coef * reg_loss
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat(
                "target_mean", (targets * mask_agent).sum().item() / mask_elems, t_env
            )
