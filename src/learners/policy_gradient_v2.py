...
class PGLearner_v2:
    ...
    def _calculate_advs(
        ...
        q_outs = torch.stack(q_outs, dim=1)  # Concat over time
        ...
        pi_taken = torch.gather(pi, dim=-1, index=actions[:, :-1]).squeeze(
            -1
        )  # [bs, t, n_agents]
        ...
        if self.args.mixer:
            targets_taken = self.mixer(
                targets_taken, batch["state"][:, :]
            )  # [bs, t, 1]
