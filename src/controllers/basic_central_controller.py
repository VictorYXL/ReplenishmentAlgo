...
class CentralBasicMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        return agent_outs.view(
            ep_batch.batch_size, self.n_agents, self.args.n_actions, -1
        )
    ...
    def save_models(self, path):
        torch.save(self.agent.state_dict(), "{}/central_agent.th".format(path))

    def load_models(self, path):
        self.agent.load_state_dict(
            torch.load(
                "{}/central_agent.th".format(path),
    ...
    def _build_inputs(self, batch, t):
        ...
        if self.args.central_agent == "central_rnn_big":
            inputs[0] = (
                batch["state"][:, t].unsqueeze(1).repeat(1, self.args.n_agents, 1)
            )  # b1av
