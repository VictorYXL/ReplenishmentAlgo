    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        with torch.no_grad():
            ...
            if len(self.buffer) < self.args.frames:
                self.buffer = [
                    torch.zeros_like(agent_inputs) for i in range(self.args.frames)
                ]
            # stack
            self.buffer = self.buffer[1:] + [agent_inputs]
            batch_agent_inputs = torch.stack(self.buffer, dim=1)  # b, t, a, c

        agent_outs, self.hidden_states = self.agent(
            batch_agent_inputs, self.hidden_states
        )
