...
def run_sequential(args, logger):
    ...
    local_args = copy.deepcopy(args)
    # local_args.use_individual_envs = True
    local_args.critic_input_seq_str = "o_d"
    local_args.learner = "local_ppo_learner"
    if local_args.max_individual_envs < 1:
        local_args.batch_size_run = max(
            1, int(local_args.max_individual_envs * args.n_agents)
    ...
    # test_runner = r_REGISTRY["local_parallel"](args=local_args, use_wandb=False)
    # test_runner.batch_size = 10
    # test_runner.setup(
    #     scheme=local_scheme,
    #     groups=local_groups,
    #     preprocess=local_preprocess,
    ...
    eval4test_args.env_args['map_name'] = "n58c1000d21s400l100"
    eval4test_args.env_args['time_limit'] = 100
    eval4test_args.env_args['max_individual_envs'] = 8
    eval4test_args.action_selector = "epsilon_greedy" #"multinomial" #"epsilon_greedy"
    eval4test_args.save_probs = False
    eval4test_args.batch_size_run = 8
    eval4test_args.batch_size = 8
    eval4test_args.buffer_size = 8
    eval4test_args.agent_ids = [1] * 8
    ...
    eval4train_args.env_args['map_name'] = "n58c1000d21s300l100"
    eval4train_args.env_args['time_limit'] = 100
    eval4train_args.env_args['max_individual_envs'] = 50
    eval4train_args.action_selector = "epsilon_greedy" #"multinomial" #"epsilon_greedy"
    eval4train_args.save_probs = False
    eval4train_args.batch_size_run = 50
    eval4train_args.batch_size = 50
    eval4train_args.buffer_size = 50
    eval4train_args.agent_ids = [1] * 50
    ...
    if args.checkpoint_path != "":
        ...
        if args.evaluate:
            ...
            agent_ids = [i+51 for i in range(8)]
            ...
            C_trajectories = np.zeros(
                (50, 3, 100 + 1), dtype=int
            )
            agent_ids = [i+1 for i in range(50)]
            ...
            for agent in range(1, 50+1):
                logger.console_logger.info(
                    "sku{} | train_return:{:.2f} | train_balance:{:.2f} |".format(
                        agent, eval4train_return[agent-1], eval4train_balance[agent-1]
                    )
                )

            for agent in range(1, 8+1):
                logger.console_logger.info(
                    "sku{} | test_return:{:.2f} | test_balance:{:.2f} |".format(
                        agent+50, eval4test_return[agent-1], eval4test_balance[agent-1], 
                    )
                )
    ...
    while local_runner.t_env <= args.t_max:  # for the_i in range(1):#
        ...
        if (local_runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            if args.use_wandb:
                for agent in range(1, 8+1):
                    wandb.log(
                        {
                        f"MixedEnvSKUReturn/eval4test_return_sku{agent+50}": eval4test_return[agent-1],
                        f"MixedEnvSKUReturn/eval4test_balance_sku{agent+50}": eval4test_balance[agent-1],
                        },
                        step=local_runner.t_env,
                    )
                for agent in range(1, 50+1):
        ...
        if args.save_model and val_best_balance < eval4train_balance.sum():
            ...
            save_path = os.path.join(
                args.local_results_path, args.unique_token, "models", "best_agent"
            )
            os.makedirs(save_path, exist_ok=True)
            logger.console_logger.info(f"Saving models to {save_path}")
        ...
        if args.save_model and (
            ...
            model_save_time = local_runner.t_env
            save_path = os.path.join(
                args.local_results_path, args.unique_token, "models", str(local_runner.t_env)
            )
            os.makedirs(save_path, exist_ok=True)
            logger.console_logger.info(f"Saving models to {save_path}")
