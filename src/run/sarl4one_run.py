...
def run_sequential(args, logger):
    ...
    args.local_results_path = os.environ.get('AMLT_OUTPUT_DIR', '.')
    ...
    # eval for timestep 400~500
    test_args = copy.deepcopy(local_args)
    test_args.env_args['map_name'] = "n58c2000d21s400l100"
    test_args.env_args['time_limit'] = 100
    test_args.action_selector = "multinomial"
    test_args.save_probs = False
    test_runner = r_REGISTRY["local_parallel"](args=test_args, use_wandb=False)
    test_mac = mac_REGISTRY[local_args.mac](
        local_buffer.scheme, local_groups, test_args
    )
    test_mac.agent = mac.agent
    ...
    test_train_args = copy.deepcopy(local_args)
    test_train_args.env_args['map_name'] = "n58c2000d21s0l400"
    test_train_args.env_args['time_limit'] = 400
    test_train_args.action_selector = "multinomial"
    test_train_args.save_probs = False
    test_train_runner = r_REGISTRY["local_parallel"](args=test_train_args, use_wandb=False)
    test_train_runner.setup(
        scheme=local_scheme,
        groups=local_groups,
        preprocess=local_preprocess,
        mac=test_mac,
    ...
    while local_runner.t_env <= args.t_max:  # for the_i in range(1):#
        ...
        for _ in range(args.n_decoupled_iterations):
            if not args.select_ls_randomly:
                ...
                agent_ids = [args.target_sku for _ in range(args.max_individual_envs)]
        ...
        if (
            ...
            visual_runner.run_visualize(visual_outputs_path, local_runner.t_env)
            if args.use_wandb:
                imgs = sorted(
                    glob.glob(
        ...
        if (local_runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            C_trajectories = np.zeros(
                (10, 3, 100 + 1), dtype=int
            )
            agent_ids = [args.target_sku for _ in range(10)]
            ...
            # test_runner.args.env_args["map_name"] = new_map_name
            C_trajectories = np.zeros(
                (10, 3, 400 + 1), dtype=int
            )
            agent_ids = [args.target_sku for _ in range(10)]
            test_train_runner.restart_ps(agent_ids)
            with torch.no_grad():
                episode_batch_train = test_train_runner.run(
                    test_mode=False, C_trajectories=C_trajectories
                )


            test_return_mean = episode_batch["reward"][:, :-1, 0].sum(1).cpu().numpy().mean()
            train_return_mean = episode_batch_train["reward"][:, :-1, 0].sum(1).cpu().numpy().mean()
            
            logger.console_logger.info(
                f"test_return_mean: {test_return_mean}"
            )
            logger.console_logger.info(
                f"train_return_mean: {train_return_mean}"
            )
            if args.use_wandb:
                wandb.log(
                    {
                     f"MixedEnvSKUReturn/test_return_mean_sku{args.target_sku}": test_return_mean,
                     f"MixedEnvSKUReturn/train_return_mean_sku{args.target_sku}": train_return_mean,
                    },
                    step=local_runner.t_env,
                )
        ...
        #     last_time = time.time()
        #     last_test_T = local_runner.t_env
        #     predefined_relative_start_days = [400]  # [10, 20, 30, 40, 50]
        #     test_episodes_returns = np.empty(
        #         [args.n_agents, len(predefined_relative_start_days)]
        #     )
        #     train_episodes_returns = np.empty(
