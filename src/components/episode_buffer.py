...
class EpisodeBatch:
    ...
    def _setup_data(self, scheme, groups, batch_size, max_seq_length, preprocess):
        ...
        for field_key, field_info in scheme.items():
            ...
            if episode_const:
                self.data.episode_data[field_key] = torch.zeros(
                    (batch_size, *shape), dtype=dtype, device=self.device
                )
            else:
                self.data.transition_data[field_key] = torch.zeros(
    ...
    def _parse_slices(self, items):
        ...
        if (
            ...
            or (
                isinstance(
                    items, (list, np.ndarray, torch.LongTensor, torch.cuda.LongTensor)
                )
            )  # [a,b,c]
        ):
